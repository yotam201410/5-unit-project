...
#                                 certfile="server.crt")
from Code.SQLManagment.SQLClient import *

sql_client = SQLClient(db_file_name="test.db")
sql_client.create_table("test", (["test", "INTEGER", "PRIMARY KEY"], ["test2", "integer", "default 0"]),
                        without_row_id=True)
# sql_client.add_data_to_table(table_name="test",rows_to_set=("test","test2"),data=(25,2))
sql_client.update_data_from_table(table_name="test", rows_to_update=("test2",), data=(5, 25), where="where test=?")
print(sql_client.get_data_from_table("test", where="where test=? and test2=?", variables=(25, 5), amount_to_fetch=1))
