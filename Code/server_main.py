...
def is_admin():
    try:
        ...
        return False
def handle_gui_messages(connected_socket,my_sockets,host_client):
    global can_connect
    while True:
        try:
...
def start_gui_connection(ip, port, my_sockets: MultiSocket, host_client: HostClient):
    ...
    while True:
        ...
        if can_connect:
            threading.Thread(target=handle_gui_messages,args=(connected_socket,my_sockets,host_client)).start()
...
def main():
    ...
    threading.Thread(target=start_gui_connection,
                     args=(my_computer.ip, Constants.server_port, my_sockets, host_client)).start()
